...
class StatsManager:
    ...
    def update_stats(self, won, time=None):
        ...
        self.save_stats()
        print(f"Обновление: победа={won}, время={time}, лучшее время={self.stats['best_time']}")
...
class MainWindow(QMainWindow):
    ...
    def win(self):
        ...
        self.stats_manager.update_stats(won="Да", time=self.DispTime.gettime())
        if self.items:
        ...
        self.MButton.win()
        print(f"Время победы: {self.DispTime.gettime()}")

    def lose(self):
        self.ingame = 0
        self.DispTime.stop()
        self.stats_manager.update_stats(won="Нет")
        for x in self.__bombs:
            self.items[x[0]][x[1]].setText("")
